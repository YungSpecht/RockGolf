...
public class Tree {
     ...
     */
    public StateVector bounce(double radiusBall, StateVector vector, double h) {
        if (Math.atan2(position[1] - vector.getYPos(), position[0] - vector.getXPos()) == 0) {
            if (position[1] - vector.getYPos() == 0)
        ...
        // Find parallel to tangent and radiusLine equation components to decompose
        // velocity
        ...
        // Decompose parallel and perpendicular to the tangent velocity components in
        // terms of x/y velocities
        ...
        System.out.println(velParallel[0] - velPerpend[0]);
        return new StateVector(vector.getXPos(), vector.getYPos(), velParallel[0] - velPerpend[0],
                velParallel[1] - velPerpend[1]);
    }
