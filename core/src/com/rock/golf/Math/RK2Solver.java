...
public class RK2Solver {
    ...
    /**
     * This method uses the Runge-Kutta 2 midpoint method to perform operations on a
     * state vector
     * object in order to give us the updated position and speed of the golf ball.
     * 
     * @param current The state vector containing the current x- and y- positions
     *                and velocities.
    ...
    public StateVector runge_kutta_two(StateVector current) {
        ...
        StateVector k2 = StateVector.multiply(function(StateVector.add(current, StateVector.multiply(k1, 2.0 / 3.0))),
                h);
        StateVector result = StateVector.add(StateVector.add(current, StateVector.multiply(k1, 1.0 / 4.0)),
                StateVector.multiply(k2, 3 / 4));
        return result;
    }
    ...
    private StateVector function(StateVector vector) {
        ...
        if (vector.getXSpeed() == 0 && vector.getYSpeed() == 0) {
            formulaX = (-PhysicsEngine.g * xSlope)
                    - uS * PhysicsEngine.g * (xSlope / Math.sqrt(Math.pow(xSlope, 2) + Math.pow(ySlope, 2)));
            formulaY = (-PhysicsEngine.g * ySlope)
                    - uS * PhysicsEngine.g * (ySlope / Math.sqrt(Math.pow(xSlope, 2) + Math.pow(ySlope, 2)));
        } else {
            formulaX = (-PhysicsEngine.g * xSlope) - uK * PhysicsEngine.g * (vector.getXSpeed()
                    / Math.sqrt(Math.pow(vector.getXSpeed(), 2) + Math.pow(vector.getYSpeed(), 2)));
            formulaY = (-PhysicsEngine.g * ySlope) - uK * PhysicsEngine.g * (vector.getYSpeed()
                    / Math.sqrt(Math.pow(vector.getXSpeed(), 2) + Math.pow(vector.getYSpeed(), 2)));
        }
