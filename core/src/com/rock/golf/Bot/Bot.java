...
/**
 * This abstract class implements all the methods and variables needed for the working of the different bots
 */
...
public abstract class Bot {
    ...
    /** 
     *
     * Gets the fitness based on Euclidean distance and if the water ends up in the water or not
     *
    ...
    public double getFitness(double[] ballPos, double[] targetPos) {
        ...
        if (engine.isInWater(ballPos[0], ballPos[1])) return Integer.MAX_VALUE;
    ...
    /** 
     *
     * Generate the collection of shots based on the angle the ball is found at w.r.t the target
     * @return shot range
     */

    public double[][][] GenerateShotRange(double currentAngle, int divergentShots, double outermostAngle, double velocityStart, double velocityEnd, int velAmount) {
        ...
        for (int i = 0; i < velAmount; i++) {
            if (velAmount == 1)
                ...
                result[0][i] = getVelocity(currentAngle, velocityStart + i * ((velocityEnd - velocityStart) / (velAmount - 1)));
        }
        for (int i = 1; i < result.length; i = i + 2) {
            for (int j = 0; j < result[0].length; j++) {
                if (velAmount == 1) {
                    result[i][j] = getVelocity(currentAngle + ((i + 1) / 2) * (outermostAngle / divergentShots), velocityStart);
                    result[i + 1][j] = getVelocity(currentAngle - ((i + 1) / 2) * (outermostAngle / divergentShots), velocityStart);
                } else {
                    result[i][j] = getVelocity(currentAngle + ((i + 1) / 2) * (outermostAngle / divergentShots), velocityStart + j * ((velocityEnd - velocityStart) / (velAmount - 1)));
                    result[i + 1][j] = getVelocity(currentAngle - ((i + 1) / 2) * (outermostAngle / divergentShots), velocityStart + j * ((velocityEnd - velocityStart) / (velAmount - 1)));
                }
            }
        }
        return result;
