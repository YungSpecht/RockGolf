...
public class RK4Solver {
        ...
        public StateVector RK4(StateVector stateVector, double t) {
                ...
                StateVector k2 = StateVector.multiply(
                                function(StateVector.add(stateVector, StateVector.multiply(k1, 1 / 2))),
                                h);
                StateVector k3 = StateVector.multiply(
                                function(StateVector.add(stateVector, StateVector.multiply(k2, 1 / 2))),
                                h);
                ...
                return StateVector.add(stateVector,
                                StateVector.multiply(
                                                StateVector.add(StateVector.add(StateVector.multiply(k2, 2),
                                                                StateVector.multiply(k3, 2)),
        ...
        private StateVector function(StateVector stateVector) {
                ...
                double formulaX = (-G * xSlope) - uK * G * (stateVector.getXSpeed()
                                / Math.sqrt(Math.pow(stateVector.getXSpeed(), 2)
                                                + Math.pow(stateVector.getYSpeed(), 2)));
